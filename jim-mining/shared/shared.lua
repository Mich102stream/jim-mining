...
function calculateDistance(p1, p2)
	return Vdist(p1.x, p1.y, p1.z, p2.x, p2.y, p2.z)
end

function spawnMinecart(coords)
	if minecartEntity then return end -- PREVENT DUPLICATES

	minecartEntity = makeProp({ prop = "k4mb1_minecart", coords = coords }, true, false)
	Wait(1000)

	AttachEntityToEntity(PlayerPedId(), minecartEntity, 20,
		0.0, 0.10, 0.5,
		0.4, 0.0, 0.0, -15.0,
		true, true, false, true, 1, true)
end

function moveMinecart(path)
	if not minecartEntity then return end

	local speed = 5.0
	currentPointIndex = 1
	Wait(3000)

	while minecartEntity do
		local nextIndex = (currentPointIndex % #path) + 1
		local distance = calculateDistance(path[currentPointIndex], path[nextIndex])
		local travelTime = distance / speed

		local startTime = GetGameTimer()
		local endTime = startTime + travelTime * 1000

		while GetGameTimer() < endTime and minecartEntity do
			local t = (GetGameTimer() - startTime) / (travelTime * 1000)

			local coords = vec3(
				lerp(path[currentPointIndex].x, path[nextIndex].x, t),
				lerp(path[currentPointIndex].y, path[nextIndex].y, t),
				lerp(path[currentPointIndex].z, path[nextIndex].z, t)
			)

			local heading = lerp(path[currentPointIndex].w, path[nextIndex].w, t)

			SetEntityCoordsNoOffset(minecartEntity, coords.x, coords.y, coords.z, true, true, true)
			SetEntityHeading(minecartEntity, heading)

			Wait(0)
		end

		if currentPointIndex >= #path - 1 then
...
function mineCartMenu(ent, right)
	...
	if ent then
		Menu[#Menu+1] = {
			...
			onSelect = function()
				local path = {}
				for i = 1, #points["Main"] do path[#path+1] = vec4(points["Main"][i].x, points["Main"][i].y, points["Main"][i].z-zadjust, points["Main"][i].w) end
				for i = 1, #points["Right"] do path[#path+1] = vec4(points["Right"][i].x, points["Right"][i].y, points["Right"][i].z-zadjust, points["Right"][i].w) end
				spawnMinecart(path[1])
				moveMinecart(path)
			end,
		}
		...
		Menu[#Menu+1] = {
			...
			onSelect = function()
				local path = {}
				for i = 1, #points["Main"] do path[#path+1] = vec4(points["Main"][i].x, points["Main"][i].y, points["Main"][i].z-zadjust, points["Main"][i].w) end
				for i = 1, #points["Left"] do path[#path+1] = vec4(points["Left"][i].x, points["Left"][i].y, points["Left"][i].z-zadjust, points["Left"][i].w) end
				spawnMinecart(path[1])
				moveMinecart(path)
			end,
		}
		...
		Menu[#Menu+1] = {
			...
			onSelect = function()
				...
				if right then
					for i = #points["Right"], 1, -1 do path[#path+1] = vec4(points["Right"][i].x, points["Right"][i].y, points["Right"][i].z-zadjust, points["Right"][i].w-180) end
				else
					for i = #points["Left"], 1, -1 do path[#path+1] = vec4(points["Left"][i].x, points["Left"][i].y, points["Left"][i].z-zadjust, points["Left"][i].w-180) end
				end

				for i = #points["Main"], 1, -1 do path[#path+1] = vec4(points["Main"][i].x, points["Main"][i].y, points["Main"][i].z-zadjust, points["Main"][i].w-180) end
